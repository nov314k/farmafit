...
G_MODULE_EXPORT on_btn_calculate_and_plot_clicked()
{
   ...
   aux_t = (char *)gtk_entry_get_text(GTK_ENTRY (example_t[0]));
   if (!digits_only(aux_t)) {
      show_msg_box (MSG_INCORRECT_ENTRY);
      gtk_widget_grab_focus (example_t[0]);
      return;
   }
   aux_p = (char *)gtk_entry_get_text (GTK_ENTRY (example_p[0]));
   if (!digits_or_single_pt_only(aux_p)) {
      show_msg_box (MSG_INCORRECT_ENTRY);
      gtk_widget_grab_focus (example_p[0]);
      return;
   }
   ...
   for (unsigned int i = 1; i < 12 && !found_the_end; ++i) {
      aux_t = (char *)gtk_entry_get_text (GTK_ENTRY (example_t[i]));
      if (!digits_only(aux_t)) {
         show_msg_box (MSG_INCORRECT_ENTRY);
         gtk_widget_grab_focus (example_t[i]);
         return;
      }
      aux_p = (char *)gtk_entry_get_text (GTK_ENTRY (example_p[i]));
      if (!digits_or_single_pt_only(aux_p)) {
         show_msg_box (MSG_INCORRECT_ENTRY);
         gtk_widget_grab_focus (example_p[i]);
         return;
      }
      if (atof(aux_t) == 0 || atof(aux_p) == 0 || strlen(aux_t) == 0 || strlen(aux_p) == 0) {
         found_the_end = 1;
      } else {
   ...
   if (numof_valid_pts >= 3) {
      ...
      gtk_widget_grab_focus (example_t[numof_valid_pts]);
   }
...
generate_plots ()
{
   ...
   double *data_time_values = (double *)g_malloc(numof_data_pts * sizeof(double));
   double *data_percentage_values = (double *)g_malloc(numof_data_pts * sizeof(double));
   SlopeSample *data_time_ticks;
   data_time_ticks = (SlopeSample *)g_malloc(numof_data_pts * sizeof(SlopeSample));
   current_data_pt = data_set;
   int max_minutes = 0;
   char *aux;
   ...
   series_md = slope_xyseries_new_filled (LBL_MEASURED_DATA, data_time_values, data_percentage_values, 12, "kor");
...
int
digits_or_single_pt_only(char *entry)
{
   int all_good_flag = 1;
   int numof_pts = 0;
