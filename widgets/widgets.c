...
gboolean
update_plot_1 (GtkWidget *button, gpointer data)
{
	for (int k = 0; k < n; k++) {
		x11[k] = (double) k * 360/100;
		y11[k] = 1 * 0.2220 * x11[k];
	}
...
gboolean
update_plot (GtkWidget *button, gpointer data)
{
	x[0]  = 0;	y[0]  = 0;
	x[1]  = 15;	y[1]  = 8.803;
	x[2]  = 30;	y[2]  = 11.935;
...
//	for (int k = 0; k < n; k++) {
//		x[k] = (double) k;//g_random_double();
//		y[k] = (double) k;//g_random_double();
//	}
	slope_view_redraw(SLOPE_VIEW(view));
	return TRUE;
}
...
{
	/* This should not be here, move it out */
	...
	/**/
  	SlopeScale  *scale = slope_xyscale_new();
	...
	/**/
	SlopeItem *series1 = slope_xyseries_new_filled ("Fitted data", x11, y11, n, "b-");
